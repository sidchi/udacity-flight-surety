...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('(airline) cannot register an Airline using registerAirline() if it is not funded', async () => {
    ...
    // ASSERT
    assert.equal(result, false, "Airline should not be able to register another airline if not FUNDED");

  });

  it('(multiparty) function call is made when multi-party threshold is reached', async () => {
    ...
    try{
        await config.flightSuretyApp.registerAirline(al1,  {from: config.owner});
        await config.flightSuretyApp.registerAirline(al2,  {from: config.owner});
        await config.flightSuretyApp.registerAirline(al3,  {from: config.owner});

        await config.flightSuretyApp.registerAirline(al4,  {from: config.owner});
    ...
    let result = await config.flightSuretyData.isAirline.call(al4); 

    // ASSERT
    assert.equal(result, false, "Multi-party call passed");
  });

  it('(airline)  operation only when funding of 10 ether', async () => {
    ...
    try{
        await config.flightSuretyApp.fund({from: al2, value: fundPrice});
        await config.flightSuretyApp.fund({from: al3, value: fundPrice});

    }catch(e){
        console.log('Funding has failed');
    }
    
    let result = await config.flightSuretyData.getAirlineOperatingStatus.call(al3);
  ...
  it('(airline) Voting is made when multi-party threshold is reached', async () => {
    ...
    try{
        ...
        let registration_status = await config.flightSuretyApp.registerAirline.call(al4,  {from: al3});
        status = await config.flightSuretyData.isAirline.call(al4);

        if(registration_status[0] == false && registration_status[1] == false){
            await config.flightSuretyApp.approveAirlineRegistration(al4, true, {from: config.owner});  //vote out
            await config.flightSuretyApp.approveAirlineRegistration(al4, true, {from: al3});         // vote in
            await config.flightSuretyApp.approveAirlineRegistration(al4, false, {from: al2});         // vote in
        }
        // Try registration again. 
        await config.flightSuretyApp.registerAirline(al4,  {from: al3});
  ...
  it('(passenger) when insured can be credited if flight is delayed', async () => {
  ...
  it('(passenger) credited can withdraw ', async () => {
